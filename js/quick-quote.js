  ...
  try {
    ...
    if (ctaBtn) {
      ctaBtn.addEventListener('click', () => {
        try { sessionStorage.setItem('qq_pending_id', ctaBtn.getAttribute('data-quick-quote-id')); } catch (_) {}
        if (typeof window.showDealForm === 'function') window.showDealForm();
        else alert('Submission form unavailable. Please refresh the page.');
      });
    }
  } catch (err) {
